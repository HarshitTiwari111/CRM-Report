...
const taskSourceLabel = (task) => {
  if (task.source === 'manual' || task.project?.name === 'My Task') return 'My Task';
  if (task.source === 'google-sheet' || task.project?.name === 'Google Sheet Task') return 'Google Sheet Task';
  return task.project?.name || 'Task';
};
  ...
  return (
    <div>
      <PageHeader
        title="My Dashboard"
        subtitle={
          cards.sheetAssigned != null && cards.myTasks != null
            ? `${cards.sheetAssigned ?? 0} sheet assigned · ${cards.myTasks ?? 0} my tasks`
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card
          title="Recent Tasks"
          actions={
            <Link to="/employee/assigned-tasks" className="text-xs font-medium text-primary-600 hover:text-primary-700">
              View all
            </Link>
          }
        >
            ...
            {recentTasks.map((task) => (
              <li key={task._id} className="flex items-center justify-between gap-3 border-b border-slate-50 dark:border-slate-700 pb-3 last:border-0 last:pb-0">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500">{taskSourceLabel(task)}</p>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <Badge color={priorityColor[task.priority] || 'slate'}>{task.priority || 'medium'}</Badge>
